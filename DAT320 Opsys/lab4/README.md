...
| Deadline:                | **September 24, 2025 23:59** |
| Grading:                 | Pass/fail |
| Submission:              | Group |
...
- The `Schedule` method is where the main scheduling logic should be implemented.
- The `Schedule` method is called by the simulator every clock tick of the system, defined via the `systime.TickDuration` constant.
- The `Schedule` method receives the current *system time* as input.
  That is, the number of ticks since the start of the simulation.
- The `Schedule` method *may assign* new jobs to the CPU before returning.

The `System` for driving the scheduling will only use the `Scheduler` interface (the `Add` and `Schedule` methods) to interact with a scheduler.
...
The `Scheduler`'s `Schedule()` method is responsible for assigning jobs to the CPU.
`Schedule()` may reassign a new job to the cpu if the `quantum` is over.
...
1. Implement the function `Started`.
...
2. Implement the functions `TurnaroundTime` and `ResponseTime` to compute the turnaround time and response time of a single job, respectively.
...
3. Implement:
   ...
   The functions `AvgTurnaroundTime` and `AvgResponseTime` to compute the average turnaround time and average response time of a schedule:
...
4. What can you say about the impact of the time quantum on the two schedulers?
