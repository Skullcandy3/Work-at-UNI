...
while ~JoyMainSwitch
    ...
    % Paramterte (Kontroll bord)
    u0 = 0;         % Intial verdi
    Kp = 0;         % start med lave verdier, typisk 0.005
    Ki = 0;         % start med lave verdier, typisk 0.005
    Kd = 0.03;      % start med lave verdier, typisk 0.001
    I_max = 100;    % Integrator begrensning
    I_min = -100;
    alfa = 0.5;     % Fast alfa slik at den kan justeres
    ...
    if k==1
        ...
        e_f(k) = e(k);
        Der_D(k) = Kd * e(k);
        D(k) = BakoverDerivasjon(Der_D(k-1:k), T_s(k));
