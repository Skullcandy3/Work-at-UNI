...
filename = 'P01_sinus_justert.mat'; 
...
while ~JoyMainSwitch
    ...
    u(k) = Lys(k) + 40; % Legger til u_A = 40 for å forhøye sinus!       
 
    if k==1
        % Spesifisering av initialverdier og parametere
        T_s(1) = 0.05;         % nominell verdi
        fc = 0.7;              % Knekk-frekvens
        tau = 1 / (2*pi*fc);   % Tau
        u_filtrert(1) = u(1);
        v_Bakover(1) = u(1);
    else
        % Beregner ulike ting filtrering og deretter derivering
        T_s(k) = Tid(k) - Tid(k-1);
        alfa(k) = 1 - exp(-T_s(k) / tau);

        % Filtrerer og deriverer med funksjoner laget 
        u_filtrert(k) = IIR_filter_lego(u_filtrert(k-1), u(k), alfa(k));
        v_Bakover(k) = BakoverDerivasjon(u(k-1:k), T_s(k));
