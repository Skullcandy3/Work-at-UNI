%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% P02_NumeriskIntegrasjonChirp
% 
% Følgende sensorer benyttes:
% - Lyssensor
%
% Motorer som benyttes:
% - Motor A
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
filename = 'P01_chirp_justert2.mat';
...
while ~JoyMainSwitch
    ...
    LysInit = -0.1; % Justerer for å få en stabil integator graf!
    u(k) = Lys(k) - LysInit;
    ...
    if online
           ...
           % Ønsker å kjøre motor ved hjelp av litt kode :)
           motorA.Speed = u_A(k);
           start(motorA)
        end
    ...
    if plotting || JoyMainSwitch
        ...
        subplot(2,1,2)
        plot(Tid(1:k),y(1:k));
        title("Trapes metoden for integrasjon av chirp")
        legend('$\{y_k\}$')
        xlabel('Tid [sek]')
        ylabel('Volum [cl]')
...
subplot(2,1,1)
plot(Tid(1:k), u(1:k))
title('Vannstr$\o$m u(t) inn og ut av balong', Interpreter='latex')
legend('$\{u_k\}$')
ylabel('Rate [l/s]')

subplot(2,1,2)
plot(Tid(1:k),y(1:k));
title('Ballongvolum y(t)')
legend('$\{y_k\}$')
xlabel('Tid [sek]')
ylabel('Volum [liter]')
